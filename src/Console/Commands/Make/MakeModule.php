...
class MakeModule extends Command
{
	...
	/**
	 * @var \HassanKerdash\Modular\Support\ModuleRegistry
	 */
	...
	public function handle()
	{
		...
		$this->newLine();
		$this->line("Run <kbd>composer update {$this->composer_name}</kbd>");
		$this->newLine();

		$this->module_registry->reload();

        exec("composer update {$this->composer_name}");
	...
	protected function shouldAbortToPublishConfig(): bool
	{
		...
		$this->info("{$this->module_namespace} module has been created.");
	...
	protected function writeStubs()
	{
		...
		$placeholders = [
			...
			'StubClassName' => strtolower($this->class_name_prefix),
			'StubComposerName' => $this->composer_name,
			'StubMigrationPrefix' => $this->module_name.'_'.time(),
	...
	protected function getStubs(): array
	{
		...
		return [
            ...
            // .gitignore
			'.gitignore' => $this->pathToStub('.gitignore'),

            // Commands
			'src/app/Console/Commands/ModuleInstallCommand.php' => $this->pathToStub('ModuleInstallCommand.php'),

            // Providers
            'src/app/Providers/ModuleServiceProvider.php' => $this->pathToStub('ServiceProvider.php'),
            'src/app/Providers/AppServiceProvider.php' => $this->pathToStub('AppServiceProvider.php'),

            // Database
			'src/database/'.$this->seedersDirectory().'/.gitkeep' => $this->pathToStub('.gitkeep'),
			'src/database/factories/.gitkeep' => $this->pathToStub('.gitkeep'),
			'src/database/migrations/.gitkeep' => $this->pathToStub('.gitkeep'),
			'src/database/seeders/DatabaseSeeder.php' => $this->pathToStub('DatabaseSeeder.php'),

            // Routes
            'src/routes/web.php' => $this->pathToStub('web-routes.php'),
            'src/routes/api.php' => $this->pathToStub('api-routes.php'),

            // Views
			'src/resources/views/.gitkeep' => $this->pathToStub('.gitkeep'),
		];
	}
